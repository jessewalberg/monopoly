...
import type {GameInfo, PlayerInfo, PropertyInfo} from './lib/prompts';
...
export const getLLMDecision = internalAction({
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    const otherPlayers: Array<PlayerInfo> = allPlayers
    ...
    const properties: Array<PropertyInfo> = allProperties.map((p) => ({
      _id: p._id,
      position: p.position,
      name: p.name,
      group: p.group,
    ...
    if (!apiKey) {
      ...
      const errorMsg =
        'OPENROUTER_API_KEY not configured in Convex Dashboard environment variables'
      console.error(`[CRITICAL] ${errorMsg}`)
      throw new Error(errorMsg)
    }
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        console.error(
          `[LLM_ERROR] Game: ${args.gameId} | Attempt ${attempt + 1}/${maxRetries}: ${errorMsg}`,
        )
        console.error(
          `[LLM_ERROR] Game: ${args.gameId} | Model: ${player.modelId}, Player: ${player.modelDisplayName}`,
        )
        ...
        if (attempt === maxRetries - 1) {
          ...
          console.log(
            `[LLM_RETRY] Game: ${args.gameId} | Retrying... attempt ${attempt + 2}/${maxRetries}`,
          )
        }
      }
    }
    ...
    let parsed = rawResponse.startsWith('[ERROR:')
      ? null
      : parseDecisionResponse(rawResponse, validActions)
    ...
    let decision =
      parsed ??
      getFallbackDecision(args.decisionType, validActions, rawResponse)

    if (parsed) {
      const validation = validateDecision(parsed, args.decisionType)
      if (!validation.valid) {
        console.error(
          `[LLM_VALIDATION] Game: ${args.gameId} | Decision validation failed: ${validation.error}`,
        )
    ...
    console.log(
      `[LLM] Game: ${args.gameId} | ${player.modelDisplayName} chose: ${decision.action} (${latencyMs}ms)`,
    )
