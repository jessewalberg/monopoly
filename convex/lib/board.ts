 ...
 */
export function getGroupPositions(group: string): readonly number[] {
 ...
 */
export function getPropertyGroups(): PropertyGroup[] {
  return [
    "brown",
    "light_blue",
    "pink",
...
export function findNearestSpace(
  ...
  const positions = type === "railroad"
    ? PROPERTY_GROUPS.railroad
    : PROPERTY_GROUPS.utility;
 ...
 */
export function getAllOwnablePositions(): number[] {
  return BOARD
    .filter((space) =>
      space.type === "property" ||
      space.type === "railroad" ||
 ...
 */
export function getPropertiesInGroup(group: string): PropertySpace[] {
