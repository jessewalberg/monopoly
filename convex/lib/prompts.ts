...
function buildPreRollPrompt(
  ...
  if (validActions.includes('trade') && otherPlayers) {
    ...
    const yourProps = properties.filter(
      (p) => p.ownerId === player._id && !p.isMortgaged,
    )
    if (yourProps.length > 0) {
      prompt +=
        yourProps.map((p) => `  - ${p.name} (${p.group})`).join('\n') + '\n'
    } else {
      prompt += `  (none)\n`
    }
    ...
    for (const opponent of otherPlayers) {
      ...
      const theirProps = properties.filter(
        (p) => p.ownerId === opponent._id && !p.isMortgaged,
      )
      prompt += `${opponent.modelDisplayName}:\n`
      if (theirProps.length > 0) {
        prompt +=
          theirProps.map((p) => `  - ${p.name} (${p.group})`).join('\n') + '\n'
      } else {
...
function buildPostRollPrompt(
  _player: PlayerInfo,
  _properties: Array<PropertyInfo>,
  validActions: Array<string>,
): string {
...
function buildTradeResponsePrompt(
  ...
  const offerProperties = Array.isArray(offer.properties)
    ? (offer.properties as Array<string>)
    : []
  if (offerProperties.length > 0) {
    prompt += `  - Properties: ${offerProperties.join(', ')}\n`
  }
  ...
  const requestProperties = Array.isArray(request.properties)
    ? (request.properties as Array<string>)
    : []
  if (requestProperties.length > 0) {
    prompt += `  - Properties: ${requestProperties.join(', ')}\n`
  }
...
function groupProperties(
  properties: Array<PropertyInfo>,
): Record<string, Array<PropertyInfo> | undefined> {
  const grouped: Record<string, Array<PropertyInfo> | undefined> = {}
