 ...
 */
function findCloseMatch(action: string, validActions: Array<string>): string | null {
...
export function getFallbackDecision(
  ...
  console.error(
    `[LLM_FALLBACK] Decision type: ${decisionType}, Valid actions: ${validActions.join(', ')}`,
  )
  if (rawResponse) {
    console.error(
      `[LLM_FALLBACK] Raw response that failed to parse: ${rawResponse.slice(0, 500)}`,
    )
  }
  ...
  if (validActions.includes(fallback.action)) {
    return fallback
  }
...
export function extractPropertyName(
  ...
  const name = parameters.propertyName || parameters.property || parameters.name
  ...
  switch (decisionType) {
    ...
    case 'post_roll_actions':
      if (
        ...
        decision.action === 'mortgage' ||
        decision.action === 'unmortgage'
      ) {
