import { HOTEL_LEVEL, JAIL_FINE, UNMORTGAGE_INTEREST } from './constants'
import { getHouseCost, getMortgageValue, getSpace, isProperty } from './board'
import {
  getMaxHousesInGroup,
  getMinHousesInGroup,
  getOwnedProperties,
...
export function canBuyProperty(
  ...
  if (player.cash < cost) {
    return {
      valid: false,
      reason: `Not enough cash. Need $${cost}, have $${player.cash}`,
    }
  }
...
export function canBuildHouse(
  ...
  if (!hasMonopoly(player._id, property.group, allProperties)) {
    return {
      valid: false,
      reason: 'Must own all properties in this color group',
    }
  }
  ...
  if (groupProps.some((p) => p.isMortgaged)) {
    return {
      valid: false,
      reason: 'Cannot build while any property in group is mortgaged',
    }
  }
  ...
  if (property.houses > minHouses) {
    return {
      valid: false,
      reason: 'Must build evenly across all properties in group',
    }
  }
...
export function canSellHouse(
  ...
  if (property.houses < maxHouses) {
    return {
      valid: false,
      reason: 'Must sell evenly across all properties in group',
    }
  }
...
export function canMortgage(
  ...
  if (groupProps.some((p) => p.houses > 0)) {
    return {
      valid: false,
      reason: 'Must sell all houses in group before mortgaging',
    }
  }
 ...
 */
export function canPayJailFine(player: PlayerState): {
  valid: boolean
  reason?: string
} {
  if (!player.inJail) {
 ...
 */
export function canUseJailCard(player: PlayerState): {
  valid: boolean
  reason?: string
} {
  if (!player.inJail) {
...
export function getValidActions(
  ...
  const actions: Array<ValidAction> = []
    ...
    case 'post_roll':
      ...
      if (landedOnProperty && !landedOnProperty.ownerId) {
        ...
        if (
          ...
          if (
            canBuyProperty(player, landedOnProperty, landedOnProperty.position)
              .valid
          ) {
            actions.push({
              type: 'buy_property',
...
function addBuildingActions(
  player: PlayerState,
  ownedProperties: Array<PropertyState>,
  allProperties: Array<PropertyState>,
  actions: Array<ValidAction>,
): void {
    ...
    if (prop.isMortgaged) {
      ...
      if (unmortgageResult.valid) {
        ...
        const unmortgageCost = Math.ceil(
          mortgageValue * (1 + UNMORTGAGE_INTEREST),
        )
        actions.push({
          type: 'unmortgage',
          position: prop.position,
...
export function canBuildAny(
  ...
  return ownedProperties.some(
    (prop) => canBuildHouse(player, prop, allProperties).valid,
  )
}
...
export function canMortgageAny(
  ...
  const ownedProperties = getOwnedProperties(player._id, allProperties)
  return ownedProperties.some(
    (prop) => canMortgage(player, prop, allProperties).valid,
  )
}
...
export interface TradeOffer {
  offerMoney: number
  offerProperties: Array<Id<'properties'>>
  offerGetOutOfJailCards: number
  requestMoney: number
  requestProperties: Array<Id<'properties'>>
...
export function canProposeTrade(
  ...
  if (proposer.getOutOfJailCards < offer.offerGetOutOfJailCards) {
    return {
      valid: false,
      reason: "Proposer doesn't have enough Get Out of Jail cards",
    }
  }
  ...
  for (const propId of offer.offerProperties) {
    ...
    if (!prop || prop.ownerId !== proposer._id) {
      return {
        valid: false,
        reason: "Proposer doesn't own one of the offered properties",
      }
    }
  ...
  if (recipient.getOutOfJailCards < offer.requestGetOutOfJailCards) {
    return {
      valid: false,
      reason: "Recipient doesn't have enough Get Out of Jail cards",
    }
  }
  ...
  for (const propId of offer.requestProperties) {
    ...
    if (!prop || prop.ownerId !== recipient._id) {
      return {
        valid: false,
        reason: "Recipient doesn't own one of the requested properties",
      }
    }
    // Cannot trade properties with houses
    if (prop.houses > 0) {
