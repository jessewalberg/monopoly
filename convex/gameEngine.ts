...
import type {PlayerState} from './lib/validation';
 ...
 */
function shuffleDeckIndices(count: number): Array<number> {
...
export const processTurnStep = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!currentTurn) {
      ...
      currentTurn = await ctx.db.get("turns", turnId)
    }
    ...
    const updatedGame = await ctx.db.get("games", args.gameId)
    if (updatedGame?.status === 'in_progress') {
      await ctx.scheduler.runAfter(
        updatedGame.config.speedMs,
        internal.gameEngine.processTurnStep,
        {
          gameId: args.gameId,
        },
...
async function processRolling(
  ...
  game: Doc<'games'>,
  player: Doc<'players'>,
  _properties: Array<Doc<'properties'>>,
  turn: Doc<'turns'>,
) {
  ...
  if (player.inJail) {
    ...
    await ctx.db.patch("turns", turn._id, {
      diceRoll: [d1, d2],
    ...
    if (isDoubles) {
      ...
      await addTurnEvent(
        ctx,
        turn._id,
        `Rolled doubles (${d1},${d2}) - out of jail!`,
      )
      ...
      await addTurnEvent(
        ctx,
        turn._id,
        `Moved to ${getSpace(newPosition).name}`,
      )
    } else {
      ...
      if (newJailTurns <= 0) {
        ...
        await addTurnEvent(
          ctx,
          turn._id,
          `Rolled (${d1},${d2}) - 3rd failed attempt, must pay fine`,
        )
        ...
        if (player.cash >= JAIL_FINE) {
          ...
          await addTurnEvent(
            ctx,
            turn._id,
            `Paid $${JAIL_FINE} jail fine - now free`,
          )

          // Player is free but doesn't move this turn (per official rules)
          await ctx.db.patch("games", game._id, { currentPhase: 'turn_end' })
          return
        } else {
          ...
          await addTurnEvent(
            ctx,
            turn._id,
            `Cannot afford $${JAIL_FINE} jail fine - BANKRUPT!`,
          )
          const properties = await ctx.db
            .query('properties')
        ...
        await ctx.db.patch("players", player._id, { jailTurnsRemaining: newJailTurns })
        await addTurnEvent(
          ctx,
          turn._id,
          `Rolled (${d1},${d2}) - still in jail (${newJailTurns} attempts remaining)`,
        )
        await ctx.db.patch("games", game._id, { currentPhase: 'turn_end' })
        return
      }
    ...
    if (newDoubles >= 3) {
      ...
      await addTurnEvent(
        ctx,
        turn._id,
        `Rolled 3rd doubles (${d1},${d2}) - GO TO JAIL!`,
      )
      await ctx.db.patch("turns", turn._id, {
    ...
    await ctx.db.patch("players", player._id, { consecutiveDoubles: newDoubles })
  ...
  await ctx.db.patch("games", game._id, { currentPhase: 'post_roll' })
}
...
async function processPostRoll(
  ...
  switch (space.type) {
    ...
    case 'utility': {
      ...
      if (!property.ownerId) {
        ...
        if (owner && !owner.isBankrupt) {
          const diceTotal = turn.diceRoll
            ? turn.diceRoll[0] + turn.diceRoll[1]
            : 7
          ...
          if (rent > 0) {
            ...
            await ctx.db.patch("players", owner._id, { cash: owner.cash + actualPayment })
            await addTurnEvent(
              ctx,
              turn._id,
              `Paid $${actualPayment} rent to ${owner.modelDisplayName}`,
            )
    ...
    case 'tax': {
      ...
      await ctx.db.patch("players", player._id, { cash: player.cash - actualPayment })
    ...
    case 'community_chest': {
      ...
      const deckLength = isChance
        ? CHANCE_CARDS.length
        : COMMUNITY_CHEST_CARDS.length
      ...
      if (deck.length === 0) {
        ...
        await addTurnEvent(
          ctx,
          turn._id,
          `${isChance ? 'Chance' : 'Community Chest'} deck reshuffled`,
        )
      }
      ...
      if (isChance) {
        await ctx.db.patch("games", game._id, { chanceDeck: remainingDeck })
      } else {
        await ctx.db.patch("games", game._id, { communityChestDeck: remainingDeck })
      }
      ...
      const result = isChance
        ...
        : (() => {
            ...
            return executeCommunityChestCard(
              card,
              playerState,
              allPlayerStates,
              properties,
      ...
      if (
        result.cashChange &&
        !result.payEachPlayer &&
        !result.collectFromEach
      ) {
        ...
        if (result.newPosition !== undefined) {
          ...
          if (
            ...
            const landedProperty = properties.find(
              (p) => p.position === result.newPosition,
            )
            if (
              landedProperty &&
              landedProperty.ownerId &&
              landedProperty.ownerId !== player._id &&
              !landedProperty.isMortgaged
            ) {
              const owner = allPlayers.find(
                (p) => p._id === landedProperty.ownerId,
              )
              if (owner && !owner.isBankrupt) {
                const diceTotal = turn.diceRoll
                  ? turn.diceRoll[0] + turn.diceRoll[1]
                ...
                if (result.useMultiplierTen && landedSpace.type === 'utility') {
                  ...
                  await addTurnEvent(
                    ctx,
                    turn._id,
                    `Card applies 10x utility multiplier!`,
                  )
                }

                if (rent > 0) {
                  // Refetch player for current cash after GO salary
                  const currentPlayer = await ctx.db.get("players", player._id)
                  const currentCash = currentPlayer?.cash ?? updatedCash
                  const actualPayment = Math.min(rent, currentCash)

                  await ctx.db.patch("players", player._id, {
                    cash: currentCash - actualPayment,
                  })
                  await ctx.db.patch("players", owner._id, {
                    cash: owner.cash + actualPayment,
                  })
                  await addTurnEvent(
                    ctx,
                    turn._id,
          ...
          // This happens with "Go Back 3 Spaces" from Chance at position 36 -> Community Chest at 33
            ...
            const deckLength2 = isChance2
              ? CHANCE_CARDS.length
              : COMMUNITY_CHEST_CARDS.length

            // Get fresh game state for deck
            const freshGame = await ctx.db.get("games", game._id)
            let deck2 = isChance2
              ? (freshGame?.chanceDeck ?? [])
              : (freshGame?.communityChestDeck ?? [])
            ...
            if (deck2.length === 0) {
              ...
              await addTurnEvent(
                ctx,
                turn._id,
                `${isChance2 ? 'Chance' : 'Community Chest'} deck reshuffled`,
              )
            }
            ...
            // Update deck in game state
            if (isChance2) {
              await ctx.db.patch("games", game._id, { chanceDeck: remainingDeck2 })
            } else {
              await ctx.db.patch("games", game._id, {
                communityChestDeck: remainingDeck2,
              })
            }

            // Execute the second card
            const freshPlayer = await ctx.db.get("players", player._id)
            const playerState2 = {
              _id: player._id,
            ...
            const allPlayerStates2 = await Promise.all(
              allPlayers.map(async (p) => {
                const fresh = await ctx.db.get("players", p._id)
                return {
                  _id: p._id,
                  cash: fresh?.cash ?? p.cash,
                  position: fresh?.position ?? p.position,
                }
              }),
            )
            ...
            const result2 = isChance2
              ...
              : (() => {
                  ...
                  addTurnEvent(
                    ctx,
                    turn._id,
                    `Drew Community Chest: "${card2.text}"`,
                  )
                  return executeCommunityChestCard(
                    card2,
                    playerState2,
            ...
            if (
              result2.cashChange &&
              !result2.payEachPlayer &&
              !result2.collectFromEach
            ) {
            ...
            if (result2.payEachPlayer) {
              const currentPlayer2 = await ctx.db.get("players", player._id)
              let remainingCash2 = currentPlayer2?.cash ?? updatedCash2
              for (const other of allPlayers) {
                if (other._id !== player._id && !other.isBankrupt) {
                  const freshOther = await ctx.db.get("players", other._id)
                  const payment = Math.min(
                    result2.payEachPlayer,
                    remainingCash2,
                  )
                  if (payment > 0) {
                    await ctx.db.patch("players", other._id, {
                      cash: (freshOther?.cash ?? other.cash) + payment,
                    })
                    remainingCash2 -= payment
                  }
                }
              }
              await ctx.db.patch("players", player._id, { cash: remainingCash2 })
            }
            if (result2.collectFromEach) {
              ...
              for (const other of allPlayers) {
                if (other._id !== player._id && !other.isBankrupt) {
                  ...
                  await ctx.db.patch("players", other._id, { cash: otherCash - payment })
                  totalCollected2 += payment
                }
              }
              if (currentPlayer2) {
                await ctx.db.patch("players", player._id, {
                  cash: currentPlayer2.cash + totalCollected2,
                })
              }
            }
          }
      ...
      if (result.payEachPlayer) {
        ...
        for (const other of allPlayers) {
          if (other._id !== player._id && !other.isBankrupt) {
            ...
            if (payment > 0) {
              await ctx.db.patch("players", other._id, { cash: other.cash + payment })
      ...
      if (result.collectFromEach) {
        ...
        if (currentPlayer) {
          await ctx.db.patch("players", player._id, {
            cash: currentPlayer.cash + totalCollected,
          })
        }
      }
      break
  ...
  const playerData = await ctx.db.get("players", player._id)
  ...
  const playerState: PlayerState = {
    ...
    jailTurnsRemaining:
      playerData?.jailTurnsRemaining ?? player.jailTurnsRemaining,
    getOutOfJailCards:
      playerData?.getOutOfJailCards ?? player.getOutOfJailCards,
...
async function processTurnEnd(
  ...
  // Check turn limit
  if (
    game.config.turnLimit &&
    game.currentTurnNumber >= game.config.turnLimit
  ) {
  ...
  const nextPlayerIndex = activePlayers.findIndex(
    (p) => p._id === nextPlayer._id,
  )
...
async function handleBankruptcy(
  ...
  for (const prop of playerProperties) {
    await ctx.db.patch("properties", prop._id, {
...
async function handleGameEnd(
  ...
  if (winnerId) {
    await ctx.db.patch("players", winnerId, {
      finalPosition: 1,
    })
  }
  ...
  await ctx.scheduler.runAfter(
    0,
    internal.statsAggregator.updateStatsAfterGame,
    {
      gameId,
    },
  )
...
export const payRent = mutation({
  ...
  handler: async (ctx, args) => {
    const payer = await ctx.db.get("players", args.payerId)
    const receiver = await ctx.db.get("players", args.receiverId)
    ...
    await ctx.db.patch("players", args.payerId, { cash: payer.cash - actualPayment })
    await ctx.db.patch("players", args.receiverId, { cash: receiver.cash + actualPayment })
    ...
    await ctx.db.insert('rentPayments', {
      ...
      amount: actualPayment,
      diceTotal:
        getSpace(property.position).type === 'utility' ? diceTotal : undefined,
...
export const buyProperty = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch("properties", args.propertyId, { ownerId: args.playerId })
...
export const runAuction = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const bid of args.bids) {
      const player = await ctx.db.get("players", bid.playerId)
      if (
        player &&
        !player.isBankrupt &&
        bid.amount > highestBid &&
        bid.amount <= player.cash
      ) {
    ...
    if (winnerId && highestBid > 0) {
      ...
      if (winner) {
        await ctx.db.patch("properties", args.propertyId, { ownerId: winnerId })
...
export const buildHouse = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch("properties", args.propertyId, { houses: newHouseCount })
    await ctx.db.patch("players", args.playerId, { cash: player.cash - actualCost })
...
export const mortgageProperty = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const player = await ctx.db.get("players", property.ownerId)
    if (!player) throw new Error('Owner not found')

    const mortgageValue = getMortgageValue(property.position)

    await ctx.db.patch("properties", args.propertyId, { isMortgaged: true })
    await ctx.db.patch("players", property.ownerId, { cash: player.cash + mortgageValue })
...
export const unmortgageProperty = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch("properties", args.propertyId, { isMortgaged: false })
    await ctx.db.patch("players", property.ownerId, { cash: player.cash - unmortgageCost })
