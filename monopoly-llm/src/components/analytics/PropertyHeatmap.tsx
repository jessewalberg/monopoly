...
export function PropertyHeatmap({
  ...
  const [selectedProperty, setSelectedProperty] = useState<PropertyStatData | null>(null);
  ...
  if (data.length === 0) {
    return (
      <div className="text-center py-8 text-slate-400">
        ...
        <p className="text-sm mt-1">Play some games to see property statistics!</p>
      </div>
    );
  }
  ...
  const groupedProperties = data.reduce<Record<string, PropertyStatData[]>>(
    (acc, prop) => {
      const group = prop.propertyGroup;
  ...
  const sortedGroups = groupOrder.filter((g) => groupedProperties[g]);
              ...
              {groupedProperties[group]
                ...
                .map((prop) => (
                  <PropertyCard
                    ...
                    maxValue={maxValue}
                    metric={currentMetric}
                    isSelected={selectedProperty?.propertyName === prop.propertyName}
    ...
    <div className="bg-slate-800 rounded-lg p-4 border border-slate-600">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          />
          <h3 className="text-lg font-bold text-white">{property.propertyName}</h3>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatItem
          label="Owner Win Rate"
          value={`${Math.round(property.ownerWinRate * 100)}%`}
        />
        <StatItem label="Times Purchased" value={property.timesPurchased.toString()} />
        <StatItem label="Times Auctioned" value={property.timesAuctioned.toString()} />
        <StatItem
...
export function TopPropertiesList({
  ...
  const sortedData = [...data].sort((a, b) => getValue(b) - getValue(a)).slice(0, limit);
  ...
  return (
    <div className="space-y-2">
      <h4 className="text-sm font-medium text-slate-400">Top Properties by {getLabel()}</h4>
