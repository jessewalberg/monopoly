...
export const AVAILABLE_MODELS: ReadonlyArray<LLMModel> = [
  // ============================================================
...
export function getModelsByProvider(provider: string): Array<LLMModel> {
  return AVAILABLE_MODELS.filter((m) => m.provider === provider)
}

export function getModelsByTier(tier: ModelTier): Array<LLMModel> {
  return AVAILABLE_MODELS.filter((m) => m.tier === tier)
}

export function getProviders(): Array<string> {
  return [...new Set(AVAILABLE_MODELS.map((m) => m.provider))]
}

// Get models sorted by cost (cheapest first)
export function getModelsSortedByCost(): Array<LLMModel> {
  return [...AVAILABLE_MODELS].sort((a, b) => {
    const costA =
      (a.costPerMillion?.input ?? 0) + (a.costPerMillion?.output ?? 0)
    const costB =
      (b.costPerMillion?.input ?? 0) + (b.costPerMillion?.output ?? 0)
    return costA - costB
  })
}
...
export const TOKEN_COLORS: ReadonlyArray<TokenColor> = [
