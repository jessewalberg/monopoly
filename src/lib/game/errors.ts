...
export const ErrorMessages: Record<string, string> = {
  ...
  [ErrorCodes.GAME_INVALID_STATUS]:
    'This action cannot be performed in the current game state.',
  [ErrorCodes.GAME_NOT_ENOUGH_PLAYERS]:
    'At least 2 players are required to start a game.',

  [ErrorCodes.PLAYER_NOT_FOUND]: 'Player not found.',
  [ErrorCodes.PLAYER_BANKRUPT]:
    'This player is bankrupt and cannot take actions.',
  [ErrorCodes.PLAYER_INSUFFICIENT_FUNDS]:
    'Not enough cash to complete this action.',
  ...
  [ErrorCodes.PROPERTY_HAS_HOUSES]:
    'Sell all houses before mortgaging this property.',
  [ErrorCodes.PROPERTY_NO_MONOPOLY]:
    'You need a complete color set to build houses.',
  [ErrorCodes.PROPERTY_NOT_BUILDABLE]:
    'Houses can only be built on color properties.',

  [ErrorCodes.TURN_NOT_FOUND]: 'Turn record not found.',

  [ErrorCodes.LLM_API_ERROR]:
    'AI model temporarily unavailable. Using fallback behavior.',
  [ErrorCodes.LLM_TIMEOUT]:
    'AI model response timed out. Using fallback behavior.',
  [ErrorCodes.LLM_PARSE_ERROR]:
    'Could not understand AI model response. Using fallback behavior.',
