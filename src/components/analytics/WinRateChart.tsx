...
export function WinRateChart({
  ...
  const getShortName = (name: string): string => {
    ...
    if (name.toLowerCase().startsWith('gemini')) {
      const versionMatch = name.match(/Gemini\s+([0-9.]+)/i)
      const version = versionMatch?.[1] ?? ''
      const rest = name.replace(/Gemini\s+[0-9.]+\s*/i, '').trim()
      const restLower = rest.toLowerCase()
      let variant = rest

      if (restLower.includes('flash') && restLower.includes('lite')) {
        variant = 'FlashL'
      } else if (restLower.includes('flash')) {
        variant = 'Flash'
      } else if (restLower.includes('pro')) {
        variant = 'Pro'
      }

      const label = `G${version}${variant ? ` ${variant}` : ''}`.trim()
...
export interface WinRateTrendChartProps {
  data: Record<string, Array<WinRateTrendPoint>>
  height?: number
}
...
export function WinRateTrendChart({
  ...
  for (let i = 1; i <= maxGameNumber; i++) {
    ...
    for (const modelId of modelIds) {
      ...
      const lastPoint = modelPoints.reduce<WinRateTrendPoint | undefined>(
        (latest, current) =>
          current.gameNumber <= i ? current : latest,
        undefined,
      )
      point[modelId] = lastPoint ? lastPoint.cumulativeWins : 0
    }
